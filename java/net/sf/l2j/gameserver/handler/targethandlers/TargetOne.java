	...
	public boolean meetCastConditions(Playable caster, Creature target, L2Skill skill, boolean isCtrlPressed)
	{
		...
		if (skill.isOffensive())
		{
			...
			if (target instanceof Playable)
			{
				...
				if (!caster.getActingPlayer().isInDuel() || caster.getActingPlayer().getDuelId() != playableTarget.getActingPlayer().getDuelId())
				{
					...
					if (caster.getActingPlayer().getFaction() == playableTarget.getActingPlayer().getFaction())
					{
						caster.sendMessage("You may not attack a player of the same faction.");
						return false;
					}
					
					if (!EventListeners.canAttack(caster, playableTarget))
						return false;
				}
			}
