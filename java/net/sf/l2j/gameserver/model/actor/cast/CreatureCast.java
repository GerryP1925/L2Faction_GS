...
public class CreatureCast<T extends Creature>
{
	...
	private final void onMagicLaunch()
	{
		...
		if (_target != _actor)
		{
			...
			// If the target reached a PEACE zone or event ended, stop the cast.
			if (_skill.isOffensive() && _actor instanceof Playable && _target instanceof Playable)
			{
				if (!_actor.getActingPlayer().isInDuel() || _actor.getActingPlayer().getDuelId() != _target.getActingPlayer().getDuelId())
				{
					...
					if (!EventListeners.canAttack((Playable)_actor, (Playable)_target))
					{
						stop();
						return;
					}
