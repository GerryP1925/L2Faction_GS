...
public abstract class Playable extends Creature
{
	...
	public boolean canCastOffensiveSkillOnPlayable(Playable target, L2Skill skill, boolean isCtrlPressed)
	{
		...
		if (targetPlayer.getProtectionBlessing() && (getActingPlayer().getStatus().getLevel() - targetPlayer.getStatus().getLevel() >= 10))
			return false;
		
		if (getActingPlayer().getProtectionBlessing() && (targetPlayer.getStatus().getLevel() - getActingPlayer().getStatus().getLevel() >= 10))
			return false;
		...
		if (getActingPlayer().getFaction() != target.getActingPlayer().getFaction() && EventListeners.canAttack(this, target))
			return true;
	...
	public boolean isAttackableBy(Creature attacker)
	{
		...
		if (attacker instanceof Playable)
		{
			...
			if (attacker.getActingPlayer().getFaction() != getActingPlayer().getFaction() && EventListeners.canAttack(attackerPlayable, this))
				return true;
			
			// One cannot be attacked if any of the two has Blessing of Protection and the other is >=10 levels higher and is PK
			if (getProtectionBlessing() && (attackerPlayable.getStatus().getLevel() - getStatus().getLevel() >= 10))
				return false;
			
			if (attackerPlayable.getProtectionBlessing() && (getStatus().getLevel() - attackerPlayable.getStatus().getLevel() >= 10))
				return false;
		}
	...
	public boolean isAttackableWithoutForceBy(Playable attacker)
	{
		...
		if (attacker.getActingPlayer().getFaction() != getActingPlayer().getFaction() && EventListeners.canAttack(attacker, this))
			return true;
	...
	public boolean canKeepAttacking(Creature target)
	{
		...
		if (target instanceof Playable)
		{
			...
			if (getActingPlayer().getFaction() != targetPlayer.getFaction() && EventListeners.canAttack(this, (Playable)target))
				return true;
